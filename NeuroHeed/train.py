...
class Trainer:
    def __init__(self, train_ds, val_ds, config, gpu_id, log_wandb=True, parallel=False):
        ...
        # Initialize model
        self.model = neuroheed(**self.config['neuroheed_params'])
    ...
    def train(self, epochs):
        ...
        for ep in range(self.start_epoch, epochs):
            ...
            for batch in pbar:
                ...
                # Validation check
                if self.config["val_every_step"] > 0 and (step + 1) % self.config["val_every_step"] == 0:
            ...
            if self.config["lr_halving"] and ((ep+1 == 25) or (no_improv_counter > 0 and no_improv_counter % HALF_LR_EPOCH == 0)):
                for param_group in self.optimizer.param_groups:
