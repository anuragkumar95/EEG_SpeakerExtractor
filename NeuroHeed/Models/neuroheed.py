...
import math
from .convtasnet_separator import TCNBlock, TCNStack


EPS = 1e-8

class neuroheed(nn.Module):
    def __init__(self, N=256, L=20, B=64, H=128, K=100, R=6, tcn_separator=False):
        ...
        if not tcn_separator:
            print(f"NeuroHeed using RNN separator")
            self.separator = rnn(N, B, H, K, R)
        else:
            print(f"NeuroHeed using TCN separator")
            self.separator = TCNSeparator(N=N, B=B, H=H, P=8, X=8, R=R, causal=False)
    ...
    def forward(self, mixture, eeg):
        ...
        mixture_w = self.encoder(mixture.squeeze(1))
        est_mask = self.separator(x=mixture_w, eeg=eeg)
...
class TCNSeparator(nn.Module):
    def __init__(self, N, B, H, P, X, R, causal=False):
        ...
        # Fusion layers
        self.fusion = nn.ModuleList([nn.Conv1d(B+64, B, 1, bias=False) for _ in range(X)])
        
        #Mask predictors
        self.prelu = nn.PReLU()
        self.mask_conv1x1 = nn.Conv1d(B, N, 1, bias=False)
        
        #TCN stack as separators
        self.tcn_stacks = nn.ModuleList([])
        for r in range(R):
            tcn_stack = TCNStack(X, P, B, H, causal=causal)
            self.tcn_stacks.append(tcn_stack)

        # #TCN blocks as separators
        # self.tcn_blocks = nn.ModuleList([])
        # for x in range(X):
        #     self.tcn_blocks.append(TCNBlock(B, H, P, dilation=2**x, causal=causal))
    ...
    def forward(self, x, eeg):
        ...
        # Process speech mixtures features
        x = self.layer_norm(x) # [M, N, K]
        x = self.bottleneck_conv1x1(x) # [M, B, K]

        # Separation using TCN blocks
        for i, tcn in enumerate(self.tcn_stacks):
            # Fuse speech and eeg features before each TCN block
            x = torch.cat((x, eeg),1)
            x  = self.fusion[i](x)
            x = tcn(x)
